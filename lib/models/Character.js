...
export default class Character {
  ...
  static async querySomething(query) {
    ...
    //function outputs WHERE = vs WHERE ~*
    const categoryCheck = () => {
      if(category[0] === 'isHuman' || category[0] === 'living' ) {
        return 'WHERE ' + `${category[0]} ` + '= $1'
      } return 'WHERE ' + `${category[0]} ` + '~* $1'
      
    }

    console.log('function', categoryCheck())
    ...
    // } else {
      ...
      //add loop 
      //add variable of concatonated ADD clauses
      // const {rows} = await pool.query(`
      //   SELECT *
      //   FROM characters
      //   WHERE ${category[0]} ~* $1
      // `, [value[0], ])
    //   return console.log('aint happenin')
